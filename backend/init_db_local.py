...
from app.models import Admin, Evaluator
...
with app.app_context():
    ...
    if not Admin.query.filter_by(login=admin_login).first():
        ...
        print(f"Admin user created: {admin_login}")
    else:
        print(f"Admin user already exists: {admin_login}")
    ...
    if existing_count == 0:
        ...
        print(f'{len(evaluators)} evaluators created successfully!')
    else:
        print(f'There are already {existing_count} evaluators in the system. No evaluators were created.')
