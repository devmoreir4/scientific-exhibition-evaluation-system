...
def distribute_works():
    ...
    for work in works:
        ...
        def can_evaluate(evaluator, work):
            # 1. Verificar se não é orientador do trabalho
            if evaluator.name == work.advisor:
                return False
            
            # 2. Verificar se não tem trabalhos na mesma subárea
        ...
        def has_expertise(evaluator, work):
            # Verificar se a área do avaliador corresponde à área do trabalho
        ...
        def has_subarea_interest(evaluator, work):
            # Só considerar subárea de interesse se o avaliador estiver na mesma área do trabalho
            if evaluator.area == work.area and evaluator.subareas and work.subarea in evaluator.subareas.split(';'):
                return True
            return False

        # Filtrar avaliadores pedagógicos elegíveis
